<?php 
 ...
 include 'db.php'; 
 $mail  = @$_GET['i'];
 $query = mysqli_query($con, "SELECT * from users where id = '$mail' limit 1");
 $row = mysqli_fetch_assoc($query);
 if($_GET['i'] == $mail){
   $query_stat = mysqli_query($con, "UPDATE users SET status = '' where id = '$mail'");
 }
 ?>
 <div class="col-10 off-2 xs-10 xs-off-1 md-12 xs-down-15 down-5 md-down-5 bottom-50" id="main-window-2">
 	<div class="col-10 off-1 md-container down-5 xs-down-10">
 		<div><span class="color-code-1 bold"><a href="mails.php" class="btn-sm color-code-1 px13"><i class="fas fa-arrow-left"></i></a>  Messages</span></div>
 		...
 		<div class="mother down-4  xs-12 xs-down-5"><span class="color-code-1 Quest px13"><span class="">Message:</span>  <span class="faded"><?php echo$row['message'] ?></span></span></div>
 		<div class="down-5  xs-12 xs-down-5 col-6 xs-down-5 md-down-3">
 			<form method="POST" action="helper.php">
 				<div>
 					...
 					<label class="px13 color-code-1 Quest">Reply This Mail</label>
